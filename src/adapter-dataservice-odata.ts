...
import { ISaveContext, ISaveBundle, ISaveResult, IKeyMapping, IServerError } from './entity-manager';
import { MetadataStore, AutoGeneratedKeyType, DataProperty } from './entity-metadata';
...
interface ODataSaveContext extends ISaveContext {
  tempKeys: EntityKey[];
  contentKeys: IEntity[];
}

export class DataServiceODataAdapter extends AbstractDataServiceAdapter {
  name: string;
  ...
  executeQuery(mappingContext: MappingContext) {
    ...
    let query = mappingContext.query as EntityQuery;
    if (query.parameters) {
  ...
  saveChanges(saveContext: ODataSaveContext, saveBundle: ISaveBundle) {
    ...
    let promise = new Promise((resolve, reject) => {
      OData.request({
        ...
        let entities: any[] = [];
        let keyMappings: IKeyMapping[] = [];
        let saveResult: ISaveResult = { entities: entities, keyMappings: keyMappings } ;
...
function transformValue(prop: DataProperty, val: any) {
  ...
  } else if ((prop.dataType as DataTypeSymbol).quoteJsonOData) {
    val = val != null ? val.toString() : val;
  }
  return val;
}

function createChangeRequests(saveContext: ODataSaveContext, saveBundle: ISaveBundle) {
  ...
  let tempKeys: EntityKey[] = [];
  let contentKeys: IEntity[] = [];
...
function updateDeleteMergeRequest(request: any, aspect: EntityAspect, routePrefix: string) {
  let uriKey: string;
...
function getUriKey(aspect: EntityAspect) {
...
function fmtProperty(prop: DataProperty, aspect: EntityAspect) {
  return (prop.dataType as DataTypeSymbol).fmtOData!(aspect.getPropertyValue(prop.name));
}

function createError(error: any, url: string) {
  // OData errors can have the message buried very deeply - and nonobviously
  // this code is tricky so be careful changing the response.body parsing.
  let result = new Error() as IServerError;
